		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tHiveConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:OaHFrWRBGsUhaAOXBb+PB3mgo+xbtlCvcIU0bL6lcWCbQ3Oo");
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				query_tHiveRow_10 = "CREATE VIEW " + context.ConsuDiagView
						+ " AS SELECT TF.Res,TF.Annee,T3.Diagnostic FROM ( SELECT COUNT(T1.Num_Consultation) AS Res, YEAR(Temps_Debut) AS Annee, C"
						+ "ode_Diag FROM " + context.RDVTable + " T1 JOIN " + context.ConsuTable
						+ " T2 ON (T1.Num_Consultation = T2.Num_Consultation) GROUP BY YEAR(Temps_Debut),Code_Diag ) TF JOIN "
						+ context.DiagTable + " T3 ON (TF.Code_Diag = T3.Code_Diag)";
...
/************************************************************************************************
 * 56344 characters generated by Talend Open Studio for Big Data on the 19 mai
 * 2022 10:04:24 CEST
 ************************************************************************************************/