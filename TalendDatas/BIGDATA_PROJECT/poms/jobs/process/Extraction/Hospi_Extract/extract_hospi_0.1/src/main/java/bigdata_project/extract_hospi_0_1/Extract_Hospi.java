...
package bigdata_project.extract_hospi_0_1;
 ...
 * @status
 */
public class Extract_Hospi implements TalendJob {
	...
	private final String jobName = "Extract_Hospi";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Extract_Hospi.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Extract_Hospi.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_BIGDATA_PROJECT_Extract_Hospi = new byte[0];
		static byte[] commonByteArray_BIGDATA_PROJECT_Extract_Hospi = new byte[0];
		...
		public int Num_Hospitalisation;

		public int getNum_Hospitalisation() {
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime * result + (int) this.Num_Hospitalisation;
		...
		public boolean equals(Object obj) {
			...
			if (this.Num_Hospitalisation != other.Num_Hospitalisation)
				return false;
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_BIGDATA_PROJECT_Extract_Hospi.length) {
					if (length < 1024 && commonByteArray_BIGDATA_PROJECT_Extract_Hospi.length == 0) {
						commonByteArray_BIGDATA_PROJECT_Extract_Hospi = new byte[1024];
					} else {
						commonByteArray_BIGDATA_PROJECT_Extract_Hospi = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_BIGDATA_PROJECT_Extract_Hospi, 0, length);
				strReturn = new String(commonByteArray_BIGDATA_PROJECT_Extract_Hospi, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_BIGDATA_PROJECT_Extract_Hospi) {

				try {

					int length = 0;

					this.Num_Hospitalisation = dis.readInt();
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				dos.writeInt(this.Num_Hospitalisation);
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									row1.Num_Hospitalisation = ParserUtils.parseTo_int(temp);
								...
								rowstate_tFileInputDelimited_1.setException(new RuntimeException(
										"Value is empty for column : 'Num_Hospitalisation' in 'row1' connection, value is invalid or this column should be nullable or have a default value."));
...
// Start of branch "row1"
	...
	public static void main(String[] args) {
		final Extract_Hospi Extract_HospiClass = new Extract_Hospi();

		int exitCode = Extract_HospiClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Extract_Hospi.class.getClassLoader()
					.getResourceAsStream("bigdata_project/extract_hospi_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Extract_Hospi.class.getClassLoader()
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Extract_Hospi");
		}
...
/************************************************************************************************
 * 55742 characters generated by Talend Open Studio for Big Data on the 9 mai
 * 2022 13:48:32 CEST
 ************************************************************************************************/